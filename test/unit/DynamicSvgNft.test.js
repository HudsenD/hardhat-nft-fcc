const { assert, expect } = require("chai")
const { getNamedAccounts, deployments, ethers, network } = require("hardhat")
const { developmentChains, networkConfig } = require("../../helper-hardhat-config")

!developmentChains.includes(network.name)
    ? describe.skip
    : describe("DynamicSvgNft", function () {
          let dynamicSvgNft, deployer
          const chainId = network.config.chainId
          beforeEach(async function () {
              deployer = (await getNamedAccounts()).deployer
              await deployments.fixture(["dynamicsvg", "mocks"])
              dynamicSvgNft = await ethers.getContract("DynamicSvgNft", deployer)
              mockV3Aggregator = await ethers.getContract("MockV3Aggregator", deployer)
          })
          describe("constructor", function () {
              it("intializes priceFeedAddresss, highSvgURI, and lowSvgURI correctly", async function () {
                  const priceFeedAddress = await dynamicSvgNft.getPriceFeedAddress()
                  const lowImageURI = await dynamicSvgNft.getLowImageURI()
                  const highImageURI = await dynamicSvgNft.getHighImageURI()
                  assert.equal(priceFeedAddress, mockV3Aggregator.address)
                  assert.equal(
                      lowImageURI,
                      "data:image/svg+xml;base64,"
                  )
                  assert.equal(
                      highImageURI,
                      "data:image/svg+xml;base64,"
                  )
              })
          })

          //   describe("svgToImageURI")

          describe("mintNft", function () {
              it("mints new nft with assigned high value and updates token counter", async function () {
                  await new Promise(async (resolve, reject) => {
                      dynamicSvgNft.once("CreatedNFT", async () => {
                          try {
                              const updatedTokenCount = await dynamicSvgNft.getTokenCounter()
                              const nftMintedHighValue = await dynamicSvgNft.s_tokenIdToHighValue("0")
                              assert.equal(updatedTokenCount.toNumber(), initialTokenCount.toNumber() + 1)
                              assert.equal(nftMintedHighValue.toString(), "1000")
                              resolve()
                          } catch (error) {
                              console.log(error)
                              reject(error)
                          }
                      })
                      const initialTokenCount = await dynamicSvgNft.getTokenCounter()
                      const mintResponse = await dynamicSvgNft.mintNft("1000")
                      await mintResponse.wait(1)
                  })
              })
          })
          //   describe("_baseURI")

          //   describe("tokenURI") you should write the tests for this function. very extensive but you got it
      })
